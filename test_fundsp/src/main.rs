...
fn main() {
    ...
    match config.sample_format() {
        cpal::SampleFormat::F32 => run::<f32>(t0, &device, &config.into()).unwrap(),
        cpal::SampleFormat::I16 => run::<i16>(t0, &device, &config.into()).unwrap(),
        cpal::SampleFormat::U16 => run::<u16>(t0, &device, &config.into()).unwrap(),
    ...
    T: SizedSample + FromSample<f64>,
{
    ...
    let f = 110.0;
    let m = 5.0;
    let c1 = oversample(sine_hz(f) * f * m + f >> sine());

    let f = 440.0;

    let c = (constant(f) >> sine()) & (2.0 * constant(f * (3.0 / 5.0))) >> sine() & c1;

    // let c = c & (constant(f * (1.75 / 1.0)) >> triangle());

    // let c = constant(f) >> triangle();
    ...
    // let c = dc(110.0) >> (soft_saw() & triangle());
