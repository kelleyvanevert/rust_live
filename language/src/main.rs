...
impl Display for Unit {
    fn fmt(&self, format: &mut Formatter<'_>) -> fmt::Result {
        use self::Unit::*;
        match self {
            Min => write!(format, "min"),
            Ms => write!(format, "ms"),
            S => write!(format, "s"),
            Khz => write!(format, "khz"),
            Hz => write!(format, "hz"),
        }
    }
}
...
impl Primitive {
    ...
    fn with_unit(self, unit: Unit) -> Self {
        match self {
...
impl Display for Primitive {
    fn fmt(&self, format: &mut Formatter<'_>) -> fmt::Result {
        ...
        match self {
            ...
            Int(val) => write!(format, "{val}"),
            Quantity((val, unit)) => write!(format, "{val}{unit}"),
            Str(val) => write!(format, "{val}"),
        }
    }
}
...
impl Display for Expr {
    fn fmt(&self, format: &mut Formatter<'_>) -> fmt::Result {
        use self::Expr::*;
        match self {
            Prim(val) => write!(format, "{}", val),
            Call(call) => write!(format, "{}", call),
            Var(id) => write!(format, "{}", id),
            ...
            Block(block) => write!(format, "{}", block),
            AnonymousFn(fun) => write!(format, "{}", fun),
        }
    }
}
...
impl Display for CallExpr {
    fn fmt(&self, format: &mut Formatter<'_>) -> fmt::Result {
        ...
        for (i, arg) in self.args.iter().enumerate() {
            write!(format, "{}{}", arg, if i + 1 == n { "" } else { ", " })?;
        }
        write!(format, ")")?;
...
impl Display for Stmt {
    fn fmt(&self, format: &mut Formatter<'_>) -> fmt::Result {
        ...
        match self {
            Skip => write!(format, ";"),
            Expr(expr) => write!(format, "{};", expr),
            Let((id, expr)) => write!(format, "let {} = {};", id, expr),
            Return(expr) => write!(format, "return {};", expr),
            Play(expr) => write!(format, "play {};", expr),
            Item(item) => write!(format, "{}", item),
        }
    }
}
...
impl Debug for ParamList {
    fn fmt(&self, format: &mut Formatter<'_>) -> fmt::Result {
        ...
        for (i, param) in self.0.iter().enumerate() {
            write!(format, "{}{}", param, if i + 1 == n { "" } else { ", " })?;
        }
...
impl Debug for AnonymousFn {
    fn fmt(&self, format: &mut Formatter<'_>) -> fmt::Result {
        write!(format, "|{}| {:?}", self.params, self.body)?;
...
impl Display for FnDecl {
    fn fmt(&self, format: &mut Formatter<'_>) -> fmt::Result {
        write!(format, "fn {}({}) {}", self.name, self.params, self.body)?;
