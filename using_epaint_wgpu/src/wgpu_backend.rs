...
impl RenderPass {
    ...
    pub fn add_textures(
        ...
        for (texture_id, image_delta) in textures.set.iter() {
            ...
            let alpha_srgb_pixels: Option<Vec<_>> = match &image_delta.image {
                epaint::ImageData::Color(_) => None,
                epaint::ImageData::Font(a) => Some(a.srgba_pixels(Some(1.0)).collect()),
            };

            let image_data: &[u8] = match &image_delta.image {
                epaint::ImageData::Color(c) => bytemuck::cast_slice(c.pixels.as_slice()),
                epaint::ImageData::Font(_) => {
            ...
            let label_base = match texture_id {
                epaint::TextureId::Managed(m) => format!("epaint_image_{}", m),
                epaint::TextureId::User(u) => format!("epaint_user_image_{}", u),
            };
    ...
    pub fn remove_textures(
        ...
        for texture_id in textures.free {
            let (texture, _binding) = self.textures.remove(&texture_id).ok_or_else(|| {
                // This can happen due to a bug in epaint, or if the user doesn't call `add_textures`
                // when required.
    ...
    pub fn update_egui_texture_from_wgpu_texture_with_sampler_options(
        ...
        if let epaint::TextureId::Managed(_) = id {
    ...
    pub fn update_buffers(
        ...
        for (i, epaint::ClippedPrimitive { primitive, .. }) in paint_jobs.iter().enumerate() {
